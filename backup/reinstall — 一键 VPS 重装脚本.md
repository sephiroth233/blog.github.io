...
## 与 leitbogioro/Tools 对比

原博客引用的 `leitbogioro/Tools` 和 `reinstall` 功能高度重合，两者均可一键重装 VPS。区别在于：

| | leitbogioro/Tools | bin456789/reinstall |
|--|---|---|
| 支持系统 | CentOS、Debian、Ubuntu、Windows | 19 种 Linux + Windows |
| 最低内存 | 约 256 MB | 256 MB（部分仅需 256 MB） |
| Windows 方式 | DD 镜像 | 官方 ISO + 自动驱动注入 |
| 手动救砖 | 无 | 支持安装中 SSH 救砖 |
| 社区活跃度 | ⭐ 约 5k | ⭐ 12.1k |

`reinstall` 支持的发行版更多、内存要求更低、救砖机制更完善，可以作为 `leitbogioro/Tools` 的补充或替代。